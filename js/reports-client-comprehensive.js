...
async function exportClientsListPDFAsBlob(clients, allCases, allOpponents) {
    ...
    clients.forEach((client, i) => {
        ...
        const rowBg = i % 2 === 0 ? '#fff7ed' : '#ffffff';
        const opponentsText = opponentNames.length > 0 ? opponentNames.join(' - ') : 'لا يوجد';
        tableRows += `<tr style="background: ${rowBg};"><td style="border: 1px solid #ddd; padding: 6px; text-align: center; font-size: 10px;">${client.name || 'غير محدد'}</td><td style="border: 1px solid #ddd; padding: 6px; text-align: center; font-size: 10px;">${client.phone || 'غير محدد'}</td><td style="border: 1px solid #ddd; padding: 6px; text-align: center; font-size: 10px;">${opponentsText}</td></tr>`;
    });
    const tempElement = document.createElement('div');
    tempElement.innerHTML = `<div style="font-family: Arial, sans-serif; direction: rtl; padding: 15px;"><div style="display: grid; grid-template-columns: 1fr 1fr 1fr; align-items: center; padding: 8px; border-bottom: 2px solid #1e40af; margin-bottom: 15px;"><div style="color: #1e40af; font-size: 14px; font-weight: bold; text-align: right;">تقرير الموكلين</div><div style="color: #666; font-size: 10px; text-align: center;">${currentDate}</div><div style="color: #666; font-size: 10px; text-align: left;">${officeName}</div></div><table style="width: 100%; border-collapse: collapse; margin-top: 10px;"><thead><tr><th style="background-color: #1e40af; color: white; padding: 8px; text-align: center; border: 1px solid #1e3a8a; font-weight: bold; font-size: 12px;">اسم الموكل</th><th style="background-color: #1e40af; color: white; padding: 8px; text-align: center; border: 1px solid #1e3a8a; font-weight: bold; font-size: 12px;">الهاتف</th><th style="background-color: #1e40af; color: white; padding: 8px; text-align: center; border: 1px solid #1e3a8a; font-weight: bold; font-size: 12px;">الخصوم</th></tr></thead><tbody>${tableRows}</tbody></table></div>`;
    const opt = { margin: 1, filename: `قائمة_الموكلين_${new Date().toISOString().split('T')[0]}.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'in', format: 'a4', orientation: 'portrait' } };
